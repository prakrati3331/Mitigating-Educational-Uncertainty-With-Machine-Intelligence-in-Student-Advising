...
async def set_recommended_stream(data: StreamData):
    ...
    try:
        ...
        logger.info(f"Stored stream data for user {user_id} - Stream: {stream}, Scores: {scores}")
...
async def get_quiz_results():
    ...
    try:
        return JSONResponse(content={"results": quiz_service.quiz_results})
    except Exception as e:
...
async def get_score_analytics():
    ...
    try:
        ...
        analytics = {
            "total_quizzes_taken": len(quiz_service.quiz_results),
            "average_scores": {},
            "quizzes_by_type": {},
            "recent_quizzes": list(quiz_service.quiz_results.values())[-5:],  # Last 5 quizzes
        }
        
        # Calculate average scores by quiz type
        scores_by_type = {}
        for result in quiz_service.quiz_results.values():
