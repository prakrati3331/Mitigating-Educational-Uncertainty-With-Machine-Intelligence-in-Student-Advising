...
def assess_risk_api():
    ...
    try:
        ...
        # -----------------------------------------------------------------
        # Sync dropout assessment to main FastAPI dashboard service
        # so it is available to the LLM (dashboard_service.user_data)
        # -----------------------------------------------------------------
        try:
            # 1) Prefer user_id from incoming data
            user_id = data.get('user_id') if isinstance(data, dict) else None

            # 2) If not provided, try to reuse the LLM's persistent user_id
            if not user_id:
                try:
                    # LLM stores user_id in LLM/user_id.txt at project root
                    project_root = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
                    llm_user_id_path = os.path.join(project_root, 'LLM', 'user_id.txt')
                    if os.path.exists(llm_user_id_path):
                        with open(llm_user_id_path, 'r', encoding='utf-8') as f:
                            user_id = f.read().strip()
                except Exception as read_err:
                    app.logger.error(f"Error reading LLM user_id.txt: {read_err}")

            # 3) Final fallback: anonymous id
            if not user_id:
            ...
            # POST to the FastAPI dashboard sync endpoint
            try:
                resp = requests.post(
                    'http://localhost:8000/api/dashboard/dropout-risk/sync',
                    json=dashboard_payload,
                    timeout=5
                )
                print("Dashboard sync status:", resp.status_code, resp.text)
            except Exception as sync_err:
                app.logger.error(f"Error syncing dropout assessment to dashboard: {sync_err}")
        except Exception as e:
            app.logger.error(f"Unexpected error preparing dashboard sync: {e}")
