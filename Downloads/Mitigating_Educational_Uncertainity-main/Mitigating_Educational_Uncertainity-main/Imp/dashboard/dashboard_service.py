...
class DashboardService:
    ...
    def _get_stream_data(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            activities = self.user_data.get(user_id, {}).get('activities', [])
            stream_activities = [
    ...
    def _get_engagement_metrics(self, user_id: str) -> Dict[str, Any]:
        """Get user engagement metrics with dummy data"""
        try:
            # Get real activities first
            user_data = self.user_data.get(user_id, {})
            real_activities = user_data.get('activities', [])
            
            # Generate dummy activities if real ones are insufficient
            if len(real_activities) < 5:
                dummy_activities = self._generate_dummy_activities(user_id)
                activities = real_activities + dummy_activities
            else:
            ...
            engagement_metrics = {
                'total_sessions': max(len(set(a.get('session_id', '') for a in activities if a.get('session_id'))), 1),
                'avg_session_duration': 25,  # Dummy: 25 minutes average
                'most_active_hour': 14,  # Dummy: 2 PM
                'most_active_day': 'Tuesday',  # Dummy
                'streak_days': 7,  # Dummy: 7 day streak
                'last_activity': activities[-1].get('timestamp') if activities else None,
                'total_activities': total_activities,
                'engagement_score': min(total_activities * 8, 100)  # Simple engagement score
            }
    ...
    def _get_course_recommendations(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check if user has any course recommendations stored
            ...
            for key in existing_keys:
                print(f"🔥 DEBUG: {key}: {user_data[key]}")
            ...
            if 'pcb_recommendations' in user_data:
                ...
                converted_pcb = []
                if 'recommendations' in pcb_data:
