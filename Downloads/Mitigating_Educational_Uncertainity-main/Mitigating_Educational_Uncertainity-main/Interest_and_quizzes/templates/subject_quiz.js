...
let currentQuizType = 'mathematics';
...
function goBack() {
    window.location.href = 'subject_quizzes.html';
}
...
style.textContent = `
    ...
    .save-progress-btn:hover {
        ...
        background: linear-gradient(135deg, #28a745, var(--success)) !important;
    }
...
async function saveProgressAndGoBack() {
    ...
    try {
        ...
        const response = await fetch('http://localhost:5001/api/quiz/progress/save', {
            ...
            body: JSON.stringify({
                user_id: localStorage.getItem('dashboard_user_id') || 'anonymous_' + Date.now(),
...
async function submitQuiz() {
    ...
    // Calculate score locally (this is accurate and what we display)
    score = 0;
    ...
    // Submit completion status to server for score tracker
    try {
        // Create a minimal submission just to mark as completed
        const answersDict = {};
        ...
        const response = await fetch(`http://localhost:5001/api/quiz/${currentQuizType}/submit`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({
                ...
                quiz_type: currentQuizType,
                answers: answersDict,
                completed_locally: true,  // Flag to indicate this was completed on client
                client_score: score,      // Send the correct score calculated by client
                client_total: questions.length
            })
        });

        if (response.ok) {
            console.log('Quiz completion submitted to server for tracking');
        } else {
            console.error('Failed to submit quiz completion to server');
        }
    } catch (error) {
        console.error('Error submitting quiz completion:', error);
        // Continue even if server submission fails - the quiz still works locally
    }

    // Show results with our locally calculated score (this is what matters)
